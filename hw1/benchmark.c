...
#define MAX_SPEED \
    54.4  // defining Bridges-2 max Gflops/s per core with peak TurboBoost
          // Frequency
...
void reference_dgemm(int N, double ALPHA, double* A, double* B, double* C) {
    ...
    int LDB = N;
    int LDC = N;
    DGEMM(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, A, &LDA, B, &LDB, &BETA, C,
          &LDC);
}
...
int main(int argc, char** argv) {
    ...
    int test_sizes[] =
        ...
        /* {31,  32,  33,  63,  64,  65,  95,  96,  97,  127,  128,  129, 159,
          160, 161, 191, 192, 193, 223, 224, 225, 255, 256, 257,  287,  288,
          289, 319, 320, 321, 351, 352, 353, 383, 384, 385, 415, 416,  417, 447,
          448, 449, 479, 480, 481, 511, 512, 513, 543, 544, 545, 575,  576, 577,
          607, 608, 609, 639, 640, 641, 671, 672, 673, 703, 704, 705,  735, 736,
          737, 767, 768, 769, 799, 800, 801, 831, 832, 833, 863, 864,  865, 895,
          896, 897, 927, 928, 929, 959, 960, 961, 991, 992, 993, 1023, 1024,
          1025};*/
    ...
    for (int isize = 0; isize < sizeof(test_sizes) / sizeof(test_sizes[0]);
        ...
        for (int n_iterations = 1; seconds < timeout; n_iterations *= 2) {
            ...
            seconds = -wall_time();
            for (int it = 0; it < n_iterations; ++it) square_dgemm(n, A, B, C);
            seconds += wall_time();
    ...
    aveper = 0;
    for (int i = 0; i < nsizes; i++) aveper += per[i];
    aveper /= nsizes * 1.0;

    /* Assigning grade based on average percentage reached (50% gets 75; 80%
     * gets 100; rest distributed proportionally) */
    if (aveper > 80)
        grade = 100.0;
    else if (aveper > 50)
        grade = (aveper - 50) * 0.25 * 100.0 / 30.0 + 75.0;
    else
        grade = aveper * 2 * 0.75;
